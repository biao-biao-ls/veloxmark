...
export default function MenuBar({ menus }: Props): React.JSX.Element {
  ...
  useEffect(() => {
    if (open === null) return
    // UX-P08 gate: close on mousedown OR click outside. A plain programmatic
    // click (and some assistive paths) never fires mousedown — without the
    // click arm, "close then reopen" sequences toggle the wrong way.
                ...
                item.separator ? (
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      // UX-P04 F4b: run the action BEFORE closing — closing
                      // first unmounts this button and resets focus to <body>,
                      // so dialog triggers can no longer recapture their
                      // invoker (openExport reads document.activeElement).
                      item.action?.()
                      setOpen(null)
                    }}
                  >
          ...
          child.separator ? (
            ...
            <button
              ...
              onClick={() => {
                ...
                // UX-P04 F4b: action first — see root item handler above.
                child.action?.()
                onPick()
              }}
            >
