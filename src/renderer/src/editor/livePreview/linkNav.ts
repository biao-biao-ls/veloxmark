...
function ensureTooltip(): HTMLDivElement | null {
  ...
  // UX-P17 F3 token-scope contract: theme tokens (--tooltip-* among them) are
  // declared ONLY on .app.theme-light/.theme-dark. A body mount sits outside
  // that ancestry, var() resolves invalid and the tooltip renders
  // transparent/black. Mount under .app when it exists (position:fixed stays
  // viewport-relative — .app carries no transform/filter/contain).
...
/** Platform mod key for tooltip hints (⌘ on Apple platforms, Ctrl elsewhere). */
...
/** UX-P17 F2: healthy-link tooltip carries the hidden-gesture hint. */
function withShortcutHint(text: string): string {
  return `${text}\n${t('link.shortcutHint', { mod: modKeyLabel() })}`
}
...
export const linkNavExtension = EditorView.domEventHandlers({
  ...
  mousemove(event, view) {
    ...
    const immediate = broken ? t('link.brokenTip') : withShortcutHint(tipTextCache.get(key) ?? href)
      ...
      if (!broken && !tipTextCache.has(key)) {
        void resolveTipText(baseDir, href).then((text) => {
          ...
          if (lastTipHref === key && tooltipEl && !tooltipEl.hidden) {
            tooltipEl.textContent = withShortcutHint(text)
          }
        })
      }
    }
