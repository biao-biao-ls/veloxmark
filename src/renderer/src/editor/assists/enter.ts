...
/** Marker-only list line: "- ", "2. ", "*   " etc. — nothing but ws after. */
const EMPTY_LIST_ITEM = /^(\s*)(?:[-*+]|\d+[.)])(\s+)$/

/**
 * Enter on an empty list item exits the list: the marker is cleared so no
 * "2. " / "- " residue remains (Typora parity; UX-P01 F2). Runs at
 * Prec.highest before insertNewlineContinueMarkup, which would otherwise
 * mint the next marker.
 */
export function exitEmptyListItem(view: EditorView): boolean {
  const { state } = view
  if (!getEditingAssists(state).enabled) return false
  let handled = false
  const changes = state.changeByRange((range) => {
    ...
    if (!m) return { range }
    // Only when the cursor sits in/after the empty item (nothing non-ws after).
    const after = line.text.slice(range.from - line.from)
    if (/\S/.test(after)) return { range }
    handled = true
    const indent = m[1]
    return {
