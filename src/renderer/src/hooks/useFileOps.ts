...
export function useFileOps({
  ...
  const notifySaveFailed = (path: string | null, err: unknown): void => {
   ...
   * rejection.
   */
  const saveAllDirtyTabs = useCallback(async (): Promise<{
    ok: boolean
  ...
  const closeTab = useCallback(
    ...
    [viewRef, activateTab, refreshTabs]
  )
