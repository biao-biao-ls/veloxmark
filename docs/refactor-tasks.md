...
- [ ] **6.7 底部栏框架**：左「+」新建（落点 = 选中目录→激活文件目录→根）/ 中当前目录名 / 右「操作」+「列表/树」两按钮 — 规格：[docs/specs/6D-bottom-bar-ops/](specs/6D-bottom-bar-ops/)
- [ ] **6.8「操作」面板顶部操作组**：新建文件 / 搜索（QuickOpen）/ 在资源管理器中显示 / 打开文件夹… / 刷新（watcher 降级兜底） — 同 6D 规格
