...
- [x] **6.11「最近使用的目录」面板段**：目录列表 + 当前根蓝点 + 置顶/移除，持久化（`lastFolderPath` 收编迁移）— 规格：[docs/specs/6F-recents-views/](specs/6F-recents-views/)
- [x] **6.12 列表/树视图渲染分叉**：列表 = 扁平文件列表（相对目录副标题）；切换即时 + 持久化；共用 6E 排序 — 同 6F 规格

> ✅ **6F 收敛记录（2026-09-24）**：T1 新叶模块 `filetree/recents.ts`（`RecentFolder {path, pinned?}` 数组恒为显示序：置顶组（相对序稳定）→ 历史组 MRU；`upsertRecent` 使用即历史首去重前移（已置顶/历史首返回**原引用**防无谓 store 通知）；`togglePinRecent`（plan 实现细化 D6：toggle 语义，取消置顶回落历史首；满 5 置顶 no-op）；`removeRecent`；`visibleRecents` 显示帽（置顶 ≤5 + 历史 ≤10，合计 ≤15）；`pathKey` 路径同一性（D8：去尾分隔符 + `\`→`/` + 小写，源字符串原样保留））+ **11 条单测**（upsert 去重移动/历史首原引用/置顶使用不动、历史挤出 10、路径同一性、置顶 toggle 序/多置顶相对序/置顶上限 5/置顶不被历史挤出、移除即时/归一命中、visibleRecents 乱序归位）。T2 store 增 `recentFolders` + 宽容 sanitize（非法条目剔除后过 `visibleRecents` 归位戴帽）；**D5 收编完成**（D7/D10）：`applyTreeRoot` 的 `patchSession({ lastFolderPath })` 写侧替换为 `upsertRecent`（根写侧唯一漏斗，plan 表点名 useTreeRoot 系同款落点修正）；`useSessionPersist` 启动**一次性迁移**（读旧槽 → upsert → 清槽，strict-mode 双跑幂等）——`lastFolderPath` 字段保留类型面零写侧。T3 `SidebarOpsPanel` 排序行后插「最近使用的目录」段（标题 + 行：📁+名 / 当前根蓝点 `.recents-current`（pathKey 比较，D4）/ hover 置顶 toggle + 移除钮，focus-visible 同显）；行点击 = `loadFolder` **6B 显式根通道**切根并关面板（AC1 不旁路根解析）；置顶/移除为行内 toggle 交互**不关面板**；面板增 max-height + 滚动（D11）。T4 列表/树渲染分叉（D2/D9）：`flattenFiles` 纯函数落 `filetreeRows.ts`（**3 条单测**：递归 relDir 副标题/平台分隔符/空树）→ FileTree 按 `fileTreeView` 分叉 rows——列表 = 全部 md 文件按 6E 当前键**全局排序**（`sortTreeNodes` 复用，纯文件列表下 groupFolders 为 no-op），行 = 文件名 + **右对齐暗色** relDir 副标题（根层无），点击/右键/active/选中与树行同路径；切换即时（偏好响应式）+ 持久化（6D store 既有）。i18n 新 5 key（`ops.recents` + `pin|unpin|remove|current`，en+zh）；`Icons.tsx` 补 `PinIcon`/`TrashIcon`。**D8 探针面登记（只增不改）**：`data-op` `sidebar.ops.recent.item`（行容器 + `data-path`）|`.open`|`.pin`|`.remove`；class `.sidebar-ops-recents*`/`.sidebar-ops-recent*`/`.recents-current`/`.filetree-list-name`/`.filetree-list-sub`。6B 根解析优先级、6E 排序、`folder:*` IPC 零触碰。收敛：typecheck 双配置 + **279 unit 全绿（+14）**；`npx madge --circular` 0 环（import 面变化）；e2e 缝零触碰（`__velox*`/命令 id 不在 diff；`loadFolder`/P13 缝签名不变）。
> 人工冒烟清单（**切根/迁移为行为变更，必做**）：① 连续「打开文件夹…」/树内下钻 3+ 目录后开「操作」面板：「最近使用的目录」段列出（📁+目录名，MRU 序）；点某行 → 面板关闭、树根切到该目录（**显式钉住**——再点开根外文件不跟随，6B D1）；② 当前树根在列表中时行尾**蓝点**；点其他行切根后蓝点跟着走；③ hover 行出置顶/移除钮（深浅主题可见）：置顶项排最前且再用历史不挤出；已置顶再点 = 取消置顶回落历史首；移除即时消失；置顶最多 5 个（再点新项无效）；历史超 10 挤出最旧未置顶；④ 置顶/移除/排序点击**均不关面板**，行点击/五操作项才关；⑤ 旧版升级：带 `lastFolderPath` 的旧会话首次启动后该目录出现在 recents 首位，重启不再重复迁移（迁移一次性）；⑥ 重启：recents 列表/置顶/移除保持（AC5/AC6）；⑦ 列表视图：点底栏「列表/树」→ 即时变扁平文件列表（不分层，行尾暗色 `subdir/` 副标题、根层无副标题）；再点回树；重启保持；列表下点行打开/右键菜单/active 高亮/选中态与树一致；⑧ 列表/树共用 6E 排序：切「按修改时间」两视图都变序（列表为全局序）；⑨ 大目录列表滚动/虚拟化不破行高；深浅主题全清晰。

### 6G. 右键菜单补齐与新建内联输入（P1）

- [x] **6.13 右键菜单补齐**：在新标签打开（P26 去重语义）/ 在资源管理器中显示 / 刷新；三档分组重排 — 规格：[docs/specs/6G-menu-inline-new/](specs/6G-menu-inline-new/)
- [x] **6.14 新建内联输入框**：树内输入名 Enter 确认/Esc 取消（复用 `filetree-renaming` 机制），`.md` 自动后缀保持，prompt 对话框退役 — 同 6G 规格

> ✅ **6G 收敛记录（2026-09-24）**：T1 `TreeMenu.tsx` `TreeMenuItem` 改 union（`{sep:true}` | `{label, danger?, op?, action}`，D6）——渲染层发 `.tree-menu-sep` 分隔线（`role="separator"`）+ `data-op={item.op}` 只挂新项（**探针面 additive-only**，既有项不带 op）；容器行为（定位/边缘翻转/Esc/外点/resize 关闭）零改动。`useWorkspaceTree` `treeMenuItems` 三档重排（D5+修订）：文件 = [在新标签打开(`tree.openInTab`→`openDocPath` P26 去重), 在资源管理器中显示(`tree.revealInOS`→`showItemInFolder` 节点路径), sep, 拷贝路径, 拷贝相对路径, sep, 重命名, 删除]；目录 = [新建文件, 新建文件夹, 在资源管理器中显示, 刷新(`tree.refresh`→`refreshTree`), sep, 拷贝路径, 拷贝相对路径, sep, 重命名, 删除]；空白/根 = [新建文件, 新建文件夹, 在资源管理器中显示, 刷新, sep, 拷贝路径]——「刷新」按 D5 修订句**只进空白+目录档**（文件档不放，D7）。T2 内联新建通道（D3/D4）：`treeNewFile`/`treeNewFolder` 的 `prompt` 体退役 → `pendingCreate {parentPath, kind}` begin-create（清 renamingPath）；`commitTreeCreate`（D8：空名=取消；非法名 alert+关沿用 `tree.invalidName`/`tree.invalidFolderName`；`pathExists` 预检重名 → 新 `tree.nameExists` alert **不静默覆盖**；`.md` 后缀仅文件+无扩展名（`withMarkdownSuffix`）；**仅后缀被追加时**建后进内联改名保持 UX-P07-F4；blur=提交对齐 RenameRow）；`cancelTreeCreate`；互斥/重置（D11：renamingPath 非空 effect 清 pendingCreate、folderPath 变更同清）。FileTree `CreateRow`（D9：与 RenameRow 全同 `.filetree-renaming`/`.filetree-rename-input` chrome、placeholder `tree.namePlaceholder`、aria 复用 `tree.newFile`/`tree.newFolder`）+ 行拼接（D10：tree 模式父目录行后插 depth=父+1，父链 fake-child 并入 renameDirs 强开；父=根/列表视图/找不到 → index 0；拼进同一 `viewRows` 数组保虚拟化数学；空树 + pendingCreate 不出 `tree.noMarkdown` 空态）。T3 i18n 新 3 key（`tree.openInTab`/`tree.nameExists`/`tree.namePlaceholder`，en+zh）+ **prompt 文案键退役**（`tree.newFileName`/`tree.newFolderName` 双字典删除，D12；`treeRename` 菜单 dialog 的 prompt 保留——规格只退役「新建」对话框）。纯函数 `withMarkdownSuffix`/`isInvalidTreeName` 导出 + **5 条单测**（D13）。**D8 探针面登记（只增不改）**：`data-op` `tree.openInTab`|`tree.revealInOS`|`tree.refresh`（仅新菜单项）；class `.tree-menu-sep`。删除确认流、拖拽移动、`openDocPath`/P26、6D 面板零触碰。收敛：typecheck 双配置 + **284 unit 全绿（+5）**；`npx madge --circular` 0 环；e2e 缝零触碰（既有 `__velox*`/`data-op`/命令 id 不在 diff）。
> 人工冒烟清单（**菜单/新建行为变更，必做**）：① 文件行右键：[在新标签打开, 在资源管理器中显示 | 拷贝路径, 拷贝相对路径 | 重命名, 删除] 三组分隔清晰；「在新标签打开」对未开文件新建 tab、已开文件只激活（P26 不复制 tab）；「在资源管理器中显示」定位到该文件；② 目录行右键：新建两项 + 资源管理器定位目录 + **刷新**（外部改动后点刷新树更新）+ 拷贝两项 + 重命名/删除；空白区右键：同新建/定位根/刷新组 + 拷贝路径（无相对路径/重命名/删除）；文件档**无刷新**（D7 偏差预期）；③ 「+」/菜单「新建文件」→ 树内出输入行（placeholder「输入名称…」）：输 `note` Enter → 建 `note.md` 并**保持内联改名**（后缀被自动追加）；输 `a.txt` Enter → 建 `a.txt` **不**进改名；Esc 取消不建；点空白处 = 提交（空名 = 取消）；④ 重名：输已存在名 Enter → 提示「名称已存在：…」且**不覆盖**、树不动；非法名（`a/b`、`..`）→ 无效名提示；⑤ 新建文件夹：无 `.md` 后缀、建后不进改名（仅文件追加后缀时进）；⑥ 内联新建与内联改名视觉一致（同输入框样式）、同屏不叠（进改名自动撤新建行）；新建行落在父目录行下一层顶部，深路径父链自动展开可见；空目录树新建第一个文件输入行可见；⑦ 回归：拷贝路径/相对路径、重命名（菜单 dialog prompt **仍在**）、删除确认流、拖拽移动、点击打开均不回归；⑧ 深浅主题下菜单分隔线/输入行清晰。
