...
>
> **清单结构**：「实施优先级」是唯一任务清单与执行顺序（checkbox 唯一来源）；「批次对照」各章是按 Markdown 元素归档的现状对照表与已领先记录，供 spec 阶段引用，不含 checkbox；文末「附录：块级 chrome 规范」是横向 UI 规范（11A 定稿），P3 立项时引用。
...
- [x] **④ 10.1 mermaid 编辑态源码/预览就地并排**（spec `10A-mermaid-edit-preview`，依赖 ③ 的双区机制）——**mermaid UX 头号差距**（已收敛 2026-09-24：**P25 底栏并联保留**——`__veloxP25` 缝契约，plan 已论证）
...
- [x] **⑥ 7.3 编辑态表格工具栏**（spec `7C-table-toolbar`，依赖 ⑤ 的 ⊞ 语义 + 下述 7.4/7.9 按钮语义，一次落地三件套）（已收敛 2026-09-24：⊞ 自 hover 栏移交至编辑浮条；+/− 把手保留——`data-table-handle` 探针契约）
...
- [x] **⑦ 9.1 语言 chip 右下角化 + 可点击切换语言**（spec `9A-code-lang-chip`）——**代码块核心交互缺口**（已收敛 2026-09-24：含 9.3 显示名；语言列表/切换机制通用挂载全部 fence，⑪ 前置达成）
...
- [x] **⑮ 7.8 表格静息态零 chrome / 与正文列对齐**（spec `7G-table-idle-chrome`，全清单风险最高项）——已收敛 2026-09-25：**负 margin 吸收**——56/28 槽宽常驻（滚动口几何）但布局效果被负 margin 抵消，表格落回正文列、死空隙消除；盒值与状态无关 → 进/出编辑 **0px 位移**（构造性证明，零抖动契约不回退且收口为 0px）；把手 z-index 抬过 gutter 条（表格 widget 的 gutter 元素无箭头/行号数字在顶部，功能零损失）；7C 工具栏同步抬一槽高，「工具栏→列把手带→表格」栈位逐像素不变，编辑态 chrome 改为 paint-only 覆盖前邻块（零位移契约必然代价，spec 已论证）。
...
- [x] **⑱ 11.9 「块级 chrome 规范」定稿**（横向收口）（spec `11A-block-chrome-spec`）——已收敛 2026-09-25：**规范附录落本文件文末**（N1 显隐四态 / N2 位置槽位 / N3 点击语义 / N4 皮肤 + 元素×态形态索引），红线 11 指向之，P3 不再逐块发明；审计闭合三发现——**A1 改码**：math hover 提示 chip 与 P06 动作按钮同锚点叠放（按钮盖 chip），chip 并入右上簇 flex 行作最右贴角元素（一排一簇，chip class/点击冒泡语义不变）；**A2 改规范**：退出 chip（面板首行右上）与语言 chip（闭栏右下）是角色槽位差异非缺陷，N2 分槽成文；**A3 改规范**：表格编辑工具栏在表头上方属交互块例外（零位移契约+基准），N2 例外条款成文。
...
11. **块级 chrome 按规范长 UI**：位置/显隐时机/点击语义以文末附录「块级 chrome 规范」（11A 定稿）为准——P3 新元素不再逐块发明；与规范不一致处要么改代码要么改规范，显式记录在对应 spec/plan。
...
## 附录：块级 chrome 规范（11A 定稿，2026-09-25）

> 适用范围：块级元素（表格/公式/代码块/mermaid 及 P3 新增元素）的 hover/focus chrome。**P3 新元素按本规范长 UI，不再逐块发明**（约束 11）；与规范不一致处要么改代码要么改规范，显式记录。行内元素仅 N1 静息态/N4 适用（8D 先例：hover 底纹即提示，无 chip 无工具栏）。

### N1 显隐时机（四态）

1. **静息**：零 chrome——无按钮/横条/边框盒/语言角标。允许不占位的透明占位（7G 把手槽、10B 透明边框）与内容自身装饰（语法高亮、圆角灰底）。
2. **hover**：浮现**一簇**——右上动作工具栏 + 提示 chip（有提示语义的元素）+ 元素专属 hover 面（8A 整行底纹带 / 8D 行内底纹 / 10B 轻边框）；移出即隐（opacity 0.15s 过渡）。
3. **聚焦/编辑**：状态 chrome（8B/P28 双区面板、7C 编辑工具栏 + 把手带、语言 chip、退出 chip）；hover 簇与之共存或按元素 AC 互斥（表格：编辑态隐藏 hover 复制条，7C AC9）。
4. **错误**：错误条浮于内容下方（琥珀色共享皮肤，mermaid/math selector-list 共享、不复制规则），右端「跳到源码」动作；错误条取代状态徽标（mermaid「更新中」徽标在报错时隐藏）。

### N2 位置槽位

- **右上簇锚点**（`top: 4px; right: 6px`）：hover 动作簇。**同锚点一排一簇**：动作按钮在左、提示 chip **最右贴角**（chip 并入 `.cm-md-block-toolbar` flex 行，不另开 absolute——11A-A1）；无提示 chip 的元素按钮直接贴角。
- **状态徽标**：左上（`top: 8px; left: 8px`），`pointer-events: none` 非交互（mermaid「更新中」先例）。
- **语言 chip**（可交互）：闭栏右下（9A 公约：absolute 锚在 fence 末行、零布局）；静息态同槽 hover 揭示（9B 同皮肤 selector-list 共享）。
- **退出 chip**（可交互）：聚焦面板**首行右上**（8B「公式 ✓」先例，dualPane 锚线 absolute 零布局）。与语言 chip 槽位不同是**角色差异**（退出 vs 语言），机制共用——11A-A2。
- **表格例外（交互块）**：编辑工具栏整条浮于表头**上方**（对齐表格列），行列把手在左/顶带、grip 在列缘（7G）；hover 复制条仅静息 hover。依据：零位移契约（约束 10）+ `table-focus.png` 基准——11A-A3。
- **行内**：无 chip、无工具栏；hover 底纹即提示（8D）。

### N3 点击语义

- **内容点击** = click-to-source（`wrapWithGap`）；表格 = 进入编辑。
- **提示 chip**：与内容点击同语义（不 stopPropagation，冒泡到 wrapWithGap）。
- **动作按钮**：`mousedown`/`click` 均 `stopPropagation`（绝不触发 click-to-source）；动作后即时反馈（复制 → ✓ 闪现 + toast，失败还原）。
- **退出 chip**：退出编辑会话（光标移出块 → 回渲染态）；Escape 同语义。
- **语言 chip**：开语言列表改写 fence info（9A）。
- **跳到源码**：定位 `$$`/fence 起点或错误行（8C/mermaid）。

### N4 皮肤

- 一律 token：hover 面 `--widget-surface`、次要文字 `--fg-dim`、按钮底 `--bg` + `--border`、按下/回显 `--accent`。**无裸色值、不新增 `.theme-dark` 补丁**（约束 6）。
- 动作按钮族：`.cm-md-block-toolbar-btn`；表格 chrome 用 `.cm-md-table-handle-btn` 族（`is-pressed` 回显列对齐）。
- hover 面/徽标/簇全部 paint-level（absolute/opacity/background/border），不推挤布局（约束 10）。

### 形态索引（元素 × 态，落地实况）

| 元素 | 静息 | hover | 聚焦/编辑 | 错误 |
|---|---|---|---|---|
| 表格 | 零 chrome、正文列对齐（7G/7H） | 复制条（右上簇） | 表头上方工具栏（⊞/对齐三键/⋮/🗑）+ 左/顶把手带 + 列宽 grip；复制条隐藏 | —（结构操作 toast） |
| 块级公式 | 居中渲染、零 chrome | 底纹带 + 一簇「复制 + 公式 </>」（N2 贴角） | 双区面板（源码+实时预览）+ 首行右上「公式 ✓」 | 错误条 + 跳到源码（8C） |
| 行内公式 | 随文渲染 | 底纹（8D） | P09 mark 显源码 | — |
| 代码块 | 圆角灰底 + 高亮、无顶栏（9B） | 右下语言角标 + 右上簇「折叠/复制」 | P28 面板 + 闭栏右下语言 chip（9A） | — |
| mermaid | 干净渲染图（10B 零盒） | 轻边框 + 右上簇「复制源码/SVG/PNG/复制图」 | 双区（10A）+ 语言 chip（10C）+ P25 底栏 | 错误条 + 跳到源码 |
